...
const errorMessages = {
    ...
    devices: 'Greška. Unjeli ste preveliki broj uređaja.'
}
    ...
    el.addEventListener('input', function(e) {
        ...
        if (sve_popunjeno()) {
            if (all_valid()) {
                ...
                if (form.dataset.function == "device-details") {
                    printDetails()
                }
            }else {
                set_all_invalid()
...
function is_valid_devices(value) {
    let value_ = parseInt(value)
    if (value_ <= (2**32)-2 && value_ > 0) return true
    return false
}
...
function all_valid() {
    ...
    form_inputs.forEach((el) => {
        if (el.dataset.type != "devices") {
            ...
            if (!is_valid_devices(el.value)) {
                if (!error_msg.includes(errorMessages.devices)) {
                    error_msg += errorMessages.devices + " "
                }
                decimal = false;
            }
        }
...
function printDetails() {
    let cidr = find_cidr(devices.value)
    let subnet_addr = subnet_calc(cidr)
    let mreza_addr = addr_network_calc(cidr)
    let first_usable_addr = first_usable(binary_addr(cidr))
    let broadcast_addr = broadcast_calc(binary_addr(cidr)).join(".")
    let last_usable = last_calc(broadcast_calc(binary_addr(cidr)))
...
function find_cidr(device_number) {
    power = 0
    while ((2**power) - 2 < device_number) {
        power++;
    }
    return 32 - power;
}
...
function broadcast_calc(value) {
    let cidr_val = find_cidr(devices.value)
